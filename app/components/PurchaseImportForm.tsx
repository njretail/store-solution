...
function downloadBulkTemplateExcel() {
  // 기존에 쓰던 양식(바코드번호 / 거래일시(년·월·일 3칸) / 상품명 / 수량 /
  // 거래액(VAT포함 원화) / 과세여부) 그대로 맞춘다. 거래일시 3칸은 파싱에서
  // 안 쓰지만(헤더 텍스트로 상품명/수량/거래액/과세만 찾음) 그대로 둬야 익숙한
  // 원래 양식과 똑같아 보인다.
  const sheetRows = [
    ["바코드번호", "거래일시", "", "", "상품명", "수량", "거래액(VAT포함 원화)", "과세여부"],
    ["8809272521532", 2026, 8, 28, "새우깡, 24개", 2, 24000, "면세"],
    ["", 2026, 8, 28, "초코파이, 12개", 3, 27000, "과세"],
  ];
  ...
  const guideRows = [
    ...
    ["3. 거래일시(년/월/일): 참고용으로만 쓰는 칸이라 비워둬도 등록에는 영향이 없습니다."],
    ["4. 상품명: 한 번에 여러 개를 묶어 산 경우 상품명 끝에 \"N개\"를 붙이세요(포장 단위)."],
    ["   예: \"새우깡, 24개\" — 낱개 24개들이 묶음이라는 뜻입니다. 신규 상품 등록 시 이 표기는"],
    ["   자동으로 제거되고 상품명만 저장됩니다."],
    ["5. 수량: 위 포장 단위를 몇 묶음 샀는지(주문 수량)입니다. 낱개 수량이 아닙니다."],
    ["   예: \"새우깡, 24개\" 2묶음을 샀다면 수량은 2 (낱개로는 48개)."],
    ["6. 거래액(VAT포함 원화): 해당 행 전체 결제금액입니다(단가가 아니라 총액)."],
    ["   매입단가 = 거래액 ÷ (포장 단위 × 수량)로 자동 계산됩니다."],
    ["   예: 거래액 24,000원 ÷ 48개 = 개당 500원."],
    ["7. 과세여부: \"과세\" 또는 \"면세\"로 입력하세요. 신규 상품 등록 시에만 사용됩니다."],
    ["8. 판매단가는 매장 기본 마진율을 적용해 자동 계산되며, 업로드 후 화면에서 직접 수정할 수 있습니다."],
  ];
